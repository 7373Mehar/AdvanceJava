...
<html>
 ...
 PreparedStatement ps=(PreparedStatement)application.getAttribute("reportdb1");
 ps.setObject(1,request.getParameter("eid"));
 ps.setObject(2,LocalDate.parse(date1));
 ps.setObject(3,LocalDate.parse(date2));
 ResultSet rst=ps.executeQuery();
 %>
 <div class='dvv'>
     <label class='hdu'>Report on date between <%=date1%> and <%=date2%></label>
  </div>
  <div class='container'>
   <div class='card col-md-12 mx-auto'>
    <div class='card-body'>
     <%if(!rst.next()){%>
       <div class='dv'>
        <label class='la' style='color:red'>No employee had gone outside between these dates</label>
       </div>
     <%}
     else
     {
